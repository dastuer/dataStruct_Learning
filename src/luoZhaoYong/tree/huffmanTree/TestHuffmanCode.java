...
//		}
	...
	private static byte[] huffmanZip(byte[] bytes) {
		...
		List<NodeData> nodes = getNodes(bytes);
		//创建一颗赫夫曼树
	 ...
	 */
	private static Map<Byte, String> getCodes(NodeData tree) {
		if(tree==null) {
			return null;
		}
	...
	private static void getCodes(NodeData node, String code, StringBuilder sb) {
	...
	private static NodeData createHuffmanTree(List<NodeData> nodes) {
		while(nodes.size()>1) {
			...
			//取出两个权值最低的二叉树
			NodeData left = nodes.get(nodes.size()-1);
			NodeData right = nodes.get(nodes.size()-2);
			//创建一颗新的二叉树
			NodeData parent = new NodeData(null, left.weight+right.weight);
